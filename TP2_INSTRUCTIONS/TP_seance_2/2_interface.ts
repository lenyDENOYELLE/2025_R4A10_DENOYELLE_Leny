...
interface Delete{
    delete(id: number): Promise<void>; // On met une promesse pour l'asynchrone
}
...
class SQLStudentRepository implements StudentRepository {
    ...
    async findOne(id: number): Promise<Student> {
        const eleve: Student = {id: 0, firstName: "", lastName: "", group: ""};
        return eleve;
    }
...
class SQLCourseRepository implements CourseRepository {
    ...
    async findOne(id: number): Promise<Course> {
        const cours: Course = {id: 0, name: "", enseignant: "", active: false};
        return cours;
    }
